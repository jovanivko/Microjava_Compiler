// generated with ast extension for cup
// version 0.8
// 25/5/2023 19:46:42


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(ClassExtend ClassExtend) { }
    public void visit(AssignDes AssignDes) { }
    public void visit(AssignList AssignList) { }
    public void visit(FormalParamDecl FormalParamDecl) { }
    public void visit(VarName VarName) { }
    public void visit(StatementList StatementList) { }
    public void visit(Assign Assign) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(ConstNameList ConstNameList) { }
    public void visit(DeclList DeclList) { }
    public void visit(Designator Designator) { }
    public void visit(GlobalMethodDecl GlobalMethodDecl) { }
    public void visit(MemberDeclList MemberDeclList) { }
    public void visit(Term Term) { }
    public void visit(ClassBody ClassBody) { }
    public void visit(Condition Condition) { }
    public void visit(ClassMethodList ClassMethodList) { }
    public void visit(MulOp MulOp) { }
    public void visit(VarNameList VarNameList) { }
    public void visit(RelOp RelOp) { }
    public void visit(ActualParamList ActualParamList) { }
    public void visit(ConstructorDeclList ConstructorDeclList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(FormalParamList FormalParamList) { }
    public void visit(ConstVal ConstVal) { }
    public void visit(Expr Expr) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(AddOp AddOp) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(ActualPars ActualPars) { }
    public void visit(Decl Decl) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(CondFact CondFact) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(FormPars FormPars) { }
    public void visit(PrintExpr PrintExpr) { }
    public void visit(ModOp ModOp) { visit(); }
    public void visit(DivOp DivOp) { visit(); }
    public void visit(MulsOp MulsOp) { visit(); }
    public void visit(SubOp SubOp) { visit(); }
    public void visit(AddsOp AddsOp) { visit(); }
    public void visit(GrtEOp GrtEOp) { visit(); }
    public void visit(GrtOp GrtOp) { visit(); }
    public void visit(LssEOp LssEOp) { visit(); }
    public void visit(LssOp LssOp) { visit(); }
    public void visit(NeqOp NeqOp) { visit(); }
    public void visit(EqOp EqOp) { visit(); }
    public void visit(AssignDerived1 AssignDerived1) { visit(); }
    public void visit(ArrayDesignator ArrayDesignator) { visit(); }
    public void visit(Desig Desig) { visit(); }
    public void visit(FuncCall FuncCall) { visit(); }
    public void visit(NewArray NewArray) { visit(); }
    public void visit(ArgCall ArgCall) { visit(); }
    public void visit(Var Var) { visit(); }
    public void visit(Const Const) { visit(); }
    public void visit(TermSingle TermSingle) { visit(); }
    public void visit(MulTerm MulTerm) { visit(); }
    public void visit(NegTermExpr NegTermExpr) { visit(); }
    public void visit(TermExpr TermExpr) { visit(); }
    public void visit(AddExpr AddExpr) { visit(); }
    public void visit(BoolExpr BoolExpr) { visit(); }
    public void visit(RelExpr RelExpr) { visit(); }
    public void visit(CondAndFact CondAndFact) { visit(); }
    public void visit(AndTerm AndTerm) { visit(); }
    public void visit(CondORFact CondORFact) { visit(); }
    public void visit(ORTerm ORTerm) { visit(); }
    public void visit(ActualParam ActualParam) { visit(); }
    public void visit(ActualParams ActualParams) { visit(); }
    public void visit(NoActuals NoActuals) { visit(); }
    public void visit(Actuals Actuals) { visit(); }
    public void visit(NoDes NoDes) { visit(); }
    public void visit(AssignDest AssignDest) { visit(); }
    public void visit(AssignSingle AssignSingle) { visit(); }
    public void visit(AssignMul AssignMul) { visit(); }
    public void visit(ProcCall ProcCall) { visit(); }
    public void visit(Decr Decr) { visit(); }
    public void visit(Incr Incr) { visit(); }
    public void visit(ErrMulAssignment ErrMulAssignment) { visit(); }
    public void visit(MulAssignment MulAssignment) { visit(); }
    public void visit(ErrAssignment ErrAssignment) { visit(); }
    public void visit(Assignment Assignment) { visit(); }
    public void visit(PrintNum PrintNum) { visit(); }
    public void visit(PrintE PrintE) { visit(); }
    public void visit(StatementBlock StatementBlock) { visit(); }
    public void visit(ForEach ForEach) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(ReturnNoExpr ReturnNoExpr) { visit(); }
    public void visit(Continue Continue) { visit(); }
    public void visit(Break Break) { visit(); }
    public void visit(While While) { visit(); }
    public void visit(IfElse IfElse) { visit(); }
    public void visit(ErrIfCond ErrIfCond) { visit(); }
    public void visit(If If) { visit(); }
    public void visit(Print Print) { visit(); }
    public void visit(Read Read) { visit(); }
    public void visit(SingleStatement SingleStatement) { visit(); }
    public void visit(NoStmt NoStmt) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(FormalParamArrayDecl FormalParamArrayDecl) { visit(); }
    public void visit(FormalParamDeclSing FormalParamDeclSing) { visit(); }
    public void visit(SingleFormalParamDecl SingleFormalParamDecl) { visit(); }
    public void visit(ErrFormalParamDecl ErrFormalParamDecl) { visit(); }
    public void visit(FormalParamDecls FormalParamDecls) { visit(); }
    public void visit(NoFormParam NoFormParam) { visit(); }
    public void visit(ErrFormParams ErrFormParams) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(MethodVoidName MethodVoidName) { visit(); }
    public void visit(MethodNonVoidName MethodNonVoidName) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(Meth Meth) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(ConstructorDecl ConstructorDecl) { visit(); }
    public void visit(Constr Constr) { visit(); }
    public void visit(Constructors Constructors) { visit(); }
    public void visit(NoMembers NoMembers) { visit(); }
    public void visit(MembersConstrNoMeth MembersConstrNoMeth) { visit(); }
    public void visit(MembersNoConstrMeth MembersNoConstrMeth) { visit(); }
    public void visit(MembersConstrMeth MembersConstrMeth) { visit(); }
    public void visit(NoClassMethods NoClassMethods) { visit(); }
    public void visit(ClassMethods ClassMethods) { visit(); }
    public void visit(NoVars NoVars) { visit(); }
    public void visit(VarDecls VarDecls) { visit(); }
    public void visit(ErrClassVarDecl ErrClassVarDecl) { visit(); }
    public void visit(ClassBodyDecl ClassBodyDecl) { visit(); }
    public void visit(ErrExtendClass ErrExtendClass) { visit(); }
    public void visit(NoClassExtend NoClassExtend) { visit(); }
    public void visit(ExtendClause ExtendClause) { visit(); }
    public void visit(ClassDeclaration ClassDeclaration) { visit(); }
    public void visit(CharConst CharConst) { visit(); }
    public void visit(BoolConst BoolConst) { visit(); }
    public void visit(NumConst NumConst) { visit(); }
    public void visit(ConstName ConstName) { visit(); }
    public void visit(GlobalConst GlobalConst) { visit(); }
    public void visit(GlobalConstDeclList GlobalConstDeclList) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(VarArrayName VarArrayName) { visit(); }
    public void visit(VarSingleName VarSingleName) { visit(); }
    public void visit(SingleVar SingleVar) { visit(); }
    public void visit(ErrVarDeclList ErrVarDeclList) { visit(); }
    public void visit(VarList VarList) { visit(); }
    public void visit(ErrVarDecl ErrVarDecl) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(DeclClass DeclClass) { visit(); }
    public void visit(DeclConst DeclConst) { visit(); }
    public void visit(DeclVar DeclVar) { visit(); }
    public void visit(NoDecl NoDecl) { visit(); }
    public void visit(Declarations Declarations) { visit(); }
    public void visit(NoGlobalMethods NoGlobalMethods) { visit(); }
    public void visit(GlobalMethods GlobalMethods) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
